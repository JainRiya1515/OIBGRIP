...
public class NumberGuessing {
    public static void main(String[] args) {
        int calculateNumber = (int) (Math.random() * 100 + 1);
        int userAnswer = 0;
        int count = 1;
        try {
            while (userAnswer != calculateNumber) {
                String response = JOptionPane.showInputDialog(null, "Enter a guess between 1 and 100");
                userAnswer = Integer.parseInt(response);
                JOptionPane.showMessageDialog(null, " " + determineGuess(userAnswer, calculateNumber, count));
                count++;
            }
        } catch (Exception e) {
